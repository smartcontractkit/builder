...
# Install go 1.12
RUN cd /usr/local && \
  curl -sS https://dl.google.com/go/go1.12.6.linux-amd64.tar.gz | tar -xz
ENV GOPATH /go
...
RUN apt-get install -y wget libgconf-2-4 --no-install-recommends \
    ...
    && apt-get update \
    && apt-get install -y google-chrome=79 fonts-ipafont-gothic fonts-wqy-zenhei fonts-thai-tlwg fonts-kacst \
